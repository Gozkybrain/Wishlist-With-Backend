...
// ? DONOT REMOVE ABOVE CODE
// # IT SERVES AS ANOTHER VERSION FOR THE API.JS
...
export default async function handler(req, res) {
    if (req.method === 'GET') {
        ...
        // Disable caching for this specific endpoint
        res.setHeader('Cache-Control', 'no-store');
        send(res, 200, todoItems);
    } else if (req.method === 'POST') {
        // Define a route to add a new task to the todoItems array
        const body = await json(req);
        const newTodo = body.task;
        todoItems.push(newTodo);
        send(res, 200, { message: 'Todo item added successfully' });
        ...
        send(res, 200, { message: 'Todo item removed successfully' });
    }
}
